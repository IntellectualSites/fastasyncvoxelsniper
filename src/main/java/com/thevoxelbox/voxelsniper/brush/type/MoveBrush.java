...
public class MoveBrush extends AbstractBrush {
	...
	public final void parameters(String[] parameters, SnipeData snipeData) {
		for (int i = 1; i < parameters.length; i++) {
			...
			if (parameter.equalsIgnoreCase("info")) {
				message.custom(ChatColor.GOLD + this.getName() + " Parameters:");
				message.custom(ChatColor.AQUA + "/b mv x[int] -- set the x direction (positive => east)");
				message.custom(ChatColor.AQUA + "/b mv y[int] -- set the y direction (positive => up)");
				message.custom(ChatColor.AQUA + "/b mv z[int] -- set the z direction (positive => south)");
				message.custom(ChatColor.AQUA + "/b mv reset -- reset the brush (x:0 y:0 z:0)");
				message.custom(ChatColor.AQUA + "Use arrow and gunpowder to define two points.");
			}
			if (parameter.equalsIgnoreCase("reset")) {
				this.moveDirections[0] = 0;
				this.moveDirections[1] = 0;
				this.moveDirections[2] = 0;
				message.custom(ChatColor.AQUA + "X direction set to: " + this.moveDirections[0]);
				message.custom(ChatColor.AQUA + "Y direction set to: " + this.moveDirections[1]);
				message.custom(ChatColor.AQUA + "Z direction set to: " + this.moveDirections[2]);
			}
	...
	private class Selection {
		 ...
		 * @throws RuntimeException Message to be sent to the player.
		 */
