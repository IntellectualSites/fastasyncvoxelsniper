...
public class PullBrush extends Brush {
	...
	private void getSurface(final SnipeData v) {
		...
		for (int z = -v.getBrushSize(); z <= v.getBrushSize(); z++) {
			...
			final int actualZ = this.getTargetBlock()
				.getZ() + z;
			for (int x = -v.getBrushSize(); x <= v.getBrushSize(); x++) {
				final double xSquared = Math.pow(x, 2);
				final int actualX = this.getTargetBlock()
					.getX() + x;
				...
				for (int x = -v.getBrushSize(); x <= v.getBrushSize(); x++) {
					...
					for (int y = -v.getBrushSize(); y <= v.getBrushSize(); y++) {
						...
						if (volume <= brushSizeSquared && this.getWorld()
							.getBlockTypeIdAt(actualX, this.getTargetBlock()
								.getY() + y, actualZ) != 0) {
							final int actualY = this.getTargetBlock()
								.getY() + y;
							...
							while (volume <= brushSizeSquared) {
								final int blockY = this.getTargetBlock()
									.getY() + y + (int) (this.vh * this.getStr(volume / brushSizeSquared));
								final int blockId = this.getWorld()
									.getBlockTypeIdAt(actualX, this.getTargetBlock()
										.getY() + y, actualZ);
