...
public class SplatterDiscBrush extends AbstractPerformerBrush {
	...
	private void splatterDisc(ToolkitProperties toolkitProperties, Block targetBlock) {
		if (this.seedPercent < SEED_PERCENT_MIN || this.seedPercent > SEED_PERCENT_MAX) {
			toolkitProperties.sendMessage(ChatColor.BLUE + "Seed percent set to: 10%");
		...
		if (this.splatterRecursions < SPLATREC_PERCENT_MIN || this.splatterRecursions > SPLATREC_PERCENT_MAX) {
			toolkitProperties.sendMessage(ChatColor.BLUE + "Recursions set to: 3");
		...
		for (int x = 2 * toolkitProperties.getBrushSize(); x >= 0; x--) {
			for (int y = 2 * toolkitProperties.getBrushSize(); y >= 0; y--) {
				if (splat[Math.max(x - 1, 0)][y] == 1 && splat[Math.min(x + 1, 2 * toolkitProperties.getBrushSize())][y] == 1 && splat[x][Math.max(0, y - 1)] == 1 && splat[x][Math.min(2 * toolkitProperties.getBrushSize(), y + 1)] == 1) {
					splat[x][y] = 1;
				}
			}
		}
		...
		for (int x = 2 * toolkitProperties.getBrushSize(); x >= 0; x--) {
			...
			for (int y = 2 * toolkitProperties.getBrushSize(); y >= 0; y--) {
				if (splat[x][y] == 1 && xSquared + Math.pow(y - toolkitProperties.getBrushSize() - 1, 2) <= rSquared) {
					this.performer.perform(targetBlock.getRelative(x - toolkitProperties.getBrushSize(), 0, y - toolkitProperties.getBrushSize()));
				}
			}
		}
	...
	public final void arrow(ToolkitProperties toolkitProperties) {
		this.splatterDisc(toolkitProperties, this.getTargetBlock());
	}
	...
	public final void powder(ToolkitProperties toolkitProperties) {
		...
		this.splatterDisc(toolkitProperties, lastBlock);
	}
	...
	public final void parameters(String[] parameters, ToolkitProperties toolkitProperties) {
		for (int i = 1; i < parameters.length; i++) {
			...
			if (parameter.equalsIgnoreCase("info")) {
				toolkitProperties.sendMessage(ChatColor.GOLD + "Splatter Disc brush Parameters:");
				toolkitProperties.sendMessage(ChatColor.AQUA + "/b sd s[int] -- set a seed percentage (1-9999). 100 = 1% Default is 1000");
				toolkitProperties.sendMessage(ChatColor.AQUA + "/b sd g[int] -- set a growth percentage (1-9999).  Default is 1000");
				toolkitProperties.sendMessage(ChatColor.AQUA + "/b sd r[int] -- set a recursion (1-10).  Default is 3");
				return;
