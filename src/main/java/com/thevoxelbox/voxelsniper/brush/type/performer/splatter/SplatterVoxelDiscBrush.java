...
public class SplatterVoxelDiscBrush extends AbstractPerformerBrush {
		...
		for (String parameter : parameters) {
			if (parameter.equalsIgnoreCase("info")) {
				snipe.createMessageSender()
					.message(ChatColor.GOLD + "Splatter Voxel Disc brush Parameters:")
					.message(ChatColor.AQUA + "/b svd s[int] -- set a seed percentage (1-9999). 100 = 1% Default is 1000")
					.message(ChatColor.AQUA + "/b svd g[int] -- set a growth percentage (1-9999).  Default is 1000")
					.message(ChatColor.AQUA + "/b svd r[int] -- set a recursion (1-10).  Default is 3")
					.send();
				return;
	...
	private void vSplatterDisc(Snipe snipe, BlockVector3 targetBlock) {
		...
		for (int x = 2 * brushSize; x >= 0; x--) {
			for (int y = 2 * brushSize; y >= 0; y--) {
				if (splat[x][y] == 1) {
					this.performer.perform(getEditSession(), blockX - brushSize + x, clampY(blockY), blockZ - brushSize + y, clampY(blockX - brushSize + x, blockY, blockZ - brushSize + y));
				}
			}
		}
	}
