...
public class RulerBrush extends AbstractBrush {
	...
	public final void powder(ToolkitProperties toolkitProperties) {
		if (this.coordinates == null || this.coordinates.lengthSquared() == 0) {
			toolkitProperties.sendMessage(ChatColor.RED + "Warning: You did not select a first coordinate with the arrow. Comparing to point 0,0,0 instead.");
			return;
		}
		toolkitProperties.sendMessage(ChatColor.BLUE + "Format = (second coord - first coord)");
		Block targetBlock = getTargetBlock();
		toolkitProperties.sendMessage(ChatColor.AQUA + "X change: " + (targetBlock.getX() - this.coordinates.getX()));
		toolkitProperties.sendMessage(ChatColor.AQUA + "Y change: " + (targetBlock.getY() - this.coordinates.getY()));
		toolkitProperties.sendMessage(ChatColor.AQUA + "Z change: " + (targetBlock.getZ() - this.coordinates.getZ()));
		Location location = targetBlock.getLocation();
		...
		toolkitProperties.sendMessage(ChatColor.AQUA + "Euclidean distance = " + distance);
		toolkitProperties.sendMessage(ChatColor.AQUA + "Block distance = " + blockDistance);
	}
	...
	public final void parameters(String[] parameters, ToolkitProperties toolkitProperties) {
		for (int i = 1; i < parameters.length; i++) {
			...
			if (parameter.equalsIgnoreCase("info")) {
				toolkitProperties.sendMessage(ChatColor.GOLD + "Ruler Brush instructions: Right click first point with the arrow. Right click with powder for distances from that block (can repeat without getting a new first block.) For placing blocks, use arrow and input the desired coordinates with parameters.");
				toolkitProperties.sendMessage(ChatColor.LIGHT_PURPLE + "/b r x[x value] y[y value] z[z value] -- Will place blocks one at a time of the type you have set with /v at the location you click + this many units away.  If you don't include a value, it will be zero.  Don't include ANY values, and the brush will just measure distance.");
				toolkitProperties.sendMessage(ChatColor.BLUE + "/b r ruler -- will reset the tool to just measure distances, not layout blocks.");
				return;
			} else if (!parameter.isEmpty() && parameter.charAt(0) == 'x') {
				this.offsetX = Integer.parseInt(parameter.replace("x", ""));
				toolkitProperties.sendMessage(ChatColor.AQUA + "X offset set to " + this.offsetX);
			} else if (!parameter.isEmpty() && parameter.charAt(0) == 'y') {
				this.offsetY = Integer.parseInt(parameter.replace("y", ""));
				toolkitProperties.sendMessage(ChatColor.AQUA + "Y offset set to " + this.offsetY);
			} else if (!parameter.isEmpty() && parameter.charAt(0) == 'z') {
				this.offsetZ = Integer.parseInt(parameter.replace("z", ""));
				toolkitProperties.sendMessage(ChatColor.AQUA + "Z offset set to " + this.offsetZ);
			} else if (parameter.startsWith("ruler")) {
				this.offsetZ = 0;
				this.offsetY = 0;
				this.offsetX = 0;
				toolkitProperties.sendMessage(ChatColor.BLUE + "Ruler mode.");
			} else {
